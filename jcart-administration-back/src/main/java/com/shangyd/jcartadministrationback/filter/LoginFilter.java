package com.shangyd.jcartadministrationback.filter;
...
//        }
        ...
        AdministrationLoginVO administrationLoginVO = null;
        try {
            administrationLoginVO = jwtUtil.verifyToken(token);
        }catch (JWTVerificationException ex){
            throw new ClientException(ClientExceptionConstant.TOKEN_INVALID_ERRCODE, ex.getMessage());
        }

        request.setAttribute("administrationId", administrationLoginVO.getAdministrationId());
        request.setAttribute("administratorUsername", administrationLoginVO.getUsername());
